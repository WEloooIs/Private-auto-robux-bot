...
async function main() {
  ...
  const supplier = createSupplier({
    supplierMode: env.supplierMode,
    supplierBaseUrl: env.supplierBaseUrl,
    notifyOperator: async (message) => {
      if (telegramBot) {
        await telegramBot.sendToOperator(message);
      } else {
        logger.warn("TELEGRAM_BOT_TOKEN or TELEGRAM_OPERATOR_CHAT_ID not set; operator message skipped");
      }
    },
    logger,
    starvellRead,
  ...
  for (const order of pendingOrders) {
    ...
    if (order.status === OrderStatus.SUPPLIER_PENDING && order.supplierOrderId) {
      queue.startPolling(order.id, order.supplierOrderId);
    }
  }
...
async function handleTelegramCommand(
  ...
  if (!trimmed.startsWith("!supplier") && !trimmed.startsWith("/balance")) return;
  ...
  if (action === "funds") {
    ...
    try {
      ...
      await reply(
        "Не удалось получить баланс автоматически. Можно задать вручную: /balance available pending frozen"
      );
    }
    return;
  }
    ...
    if (balanceService) {
      try {
        ...
        if (balance.available >= cost) {
          await prisma.order.update({
            where: { id: task.orderId },
            data: { status: OrderStatus.READY_TO_BUY, statusDetails: "QUOTE_OK" },
          });
          if (queue) {
